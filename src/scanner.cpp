...
scanner::scanner(string& code):code(code), start(0), current(0), line(1)
{
	...
	this->keyword_table.insert(make_pair("start", Token(LEFT_BRACE, "start", iteral{}, 0)));
	this->keyword_table.insert(make_pair("end", Token(RIGHT_BRACE, "end", iteral{}, 0)));

	iteral it{};
	it.boolean = false;
	this->keyword_table.insert(make_pair("false", Token(FALSE, "false", it, 0)));
	
	this->keyword_table.insert(make_pair("fn", Token(FUN, "fn", iteral{}, 0)));
